...
<script>
  import { selectedEntityDetailsStore, worldSummaryStore, gameStatusStore } from '../stores/worldStore.js';
  
  $: selectedEntity = $selectedEntityDetailsStore;
  $: worldSummary = $worldSummaryStore;
  $: gameStatus = $gameStatusStore;
  
  // Phase 0: Mock data for display testing
  let mockEntity = {
    entityId: 1,
    components: {
      tag: { name: 'marine_sarge' },
      isMarine: { name: 'Sarge', rank: 'Sergeant' },
      position: { roomId: 'docking_bay' },
      health: { current: 100, max: 100 },
      inventory: { items: [], capacity: 6 },
      personality: { type: 'leader', traits: { leadership: 9, caution: 7 } }
    },
    availableActions: [
      { type: 'moveTo', label: 'Move to adjacent room', requiresTarget: true },
      { type: 'searchArea', label: 'Search current area', requiresTarget: false }
    ]
  };
...
<div class="info-panel">
  ...
  <div class="panel-content">
    ...
    <!-- Phase 0: Mock data display for testing -->
    <div class="debug-section">
      <details>
        <summary>Phase 0 Debug - Mock Entity</summary>
        <div class="mock-entity-display">
          <h4>Mock Marine Entity (Sarge)</h4>
          <div class="components-list">
            {#each Object.entries(mockEntity.components) as [componentType, componentData]}
              <div class="component-item {getComponentTypeClass(componentType)}">
                <div class="component-header">
                  <span class="component-type">{componentType}</span>
                </div>
